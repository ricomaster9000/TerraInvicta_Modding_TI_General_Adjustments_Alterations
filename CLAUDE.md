...
- Patch classes call `setConfigVariables()` once at load to cache config values into statics; some patches read `Config.GetValueAsFloat` on every call instead. Either is accepted.
- `Config` types values by parsing: integer-looking → `int`, has `.` → `float`, `true/false` → `bool`, else string. `enable_debug_mode` is special-cased into `Config.isDebugModeActive`. Use `Config.isKeySet` before reading optional keys.
